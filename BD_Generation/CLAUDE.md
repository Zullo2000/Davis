...
- Merge to `main` with `--no-ff`, then tag (`v0.1.0`, `v0.3.0`, etc.)

## Context management
- After every 3-4 tool calls, estimate whether you are approaching context limits. If the conversation has been going on for a long time (many tool calls, large file reads) and the context is near 80%, proactively warn the user so they can run `/create-handoff` to continue in a fresh session.
